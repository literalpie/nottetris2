-- various patches for love 0.7.2 behavior
-- hopefully eventually the code will not need them
function newBody(world, x, y, m, i)
  local body = love.physics.newBody( world, x, y ) -- yes all bodies are static temporarily
  body:setMass(m)
  body:setInertia(i)
  return body
end

function sMFSreplacement(body) -- but sometimes bodies aren't static
  body:resetMassData()
  body:setType("dynamic")
end

function newShapeFnFactory(name) -- all the patches are generally the same
  return function (body, ...)
    local shape = love.physics[name](...)
    local fixture = love.physics.newFixture(body, shape)
    return fixture -- you wouldn't think this is the answer, but it is
  end
end

local newPolygonShape = newShapeFnFactory("newPolygonShape")
local newRectangleShape = newShapeFnFactory("newRectangleShape")
...
function gameA_load()
	...
	wallbodies = newBody(world, 32, -64, 0, 0) --WALLS
...
function createtetriA(i, uniqueid, x, y) --creates block, including body, shapes, image, imagedata and whatnot.
	...
	if i == 1 then --I
		tetribodies[uniqueid] = newBody(world, x, y, 0, blockrot)
		tetrishapes[uniqueid][1] = newRectangleShape( tetribodies[uniqueid], -48,0, 32, 32)
		tetrishapes[uniqueid][2] = newRectangleShape( tetribodies[uniqueid], -16,0, 32, 32)
		tetrishapes[uniqueid][3] = newRectangleShape( tetribodies[uniqueid], 16,0, 32, 32)
		tetrishapes[uniqueid][4] = newRectangleShape( tetribodies[uniqueid], 48,0, 32, 32)

	elseif i == 2 then --J
		tetribodies[uniqueid] = newBody(world, x, y, 0, blockrot)
		tetrishapes[uniqueid][1] = newRectangleShape( tetribodies[uniqueid], -32,-16, 32, 32)
		tetrishapes[uniqueid][2] = newRectangleShape( tetribodies[uniqueid], 0,-16, 32, 32)
		tetrishapes[uniqueid][3] = newRectangleShape( tetribodies[uniqueid], 32,-16, 32, 32)
		tetrishapes[uniqueid][4] = newRectangleShape( tetribodies[uniqueid], 32,16, 32, 32)
	...
	elseif i == 4 then --O
		tetribodies[uniqueid] = newBody(world, x, y, 0, blockrot)
		tetrishapes[uniqueid][1] = newRectangleShape( tetribodies[uniqueid], -16,-16, 32, 32)
		tetrishapes[uniqueid][2] = newRectangleShape( tetribodies[uniqueid], -16,16, 32, 32)
		tetrishapes[uniqueid][3] = newRectangleShape( tetribodies[uniqueid], 16,16, 32, 32)
		tetrishapes[uniqueid][4] = newRectangleShape( tetribodies[uniqueid], 16,-16, 32, 32)
	...
	elseif i == 5 then --S
		...
		tetrishapes[uniqueid][2] = newRectangleShape( tetribodies[uniqueid], 0,-16, 32, 32)
		tetrishapes[uniqueid][3] = newRectangleShape( tetribodies[uniqueid], 32,-16, 32, 32)
		tetrishapes[uniqueid][4] = newRectangleShape( tetribodies[uniqueid], 0,16, 32, 32)
	...
	elseif i == 7 then --Z
		tetribodies[uniqueid] = newBody(world, x, y, 0, blockrot)
		tetrishapes[uniqueid][1] = newRectangleShape( tetribodies[uniqueid], 0,16, 32, 32)
		tetrishapes[uniqueid][2] = newRectangleShape( tetribodies[uniqueid], 0,-16, 32, 32)
		tetrishapes[uniqueid][3] = newRectangleShape( tetribodies[uniqueid], 32,16, 32, 32)
		tetrishapes[uniqueid][4] = newRectangleShape( tetribodies[uniqueid], -32,-16, 32, 32)
	...
	sMFSreplacement(tetribodies[uniqueid])
			...
			if #tetrishapescopy == 0 then --body empty
				...
				for a = 1, numberofgroups do
					if a == 1 then --reassign the old bodyid
						...
						tetribodies[i-ioffset]:destroy()
						tetribodies[i-ioffset] = newBody(world, tetribodies[i-ioffset]:getX(), tetribodies[i-ioffset]:getY(), tetribodies[i-ioffset]:getMass(), blockrot)
						...
						--mass confusion
						sMFSreplacement(tetribodies[i-ioffset])
						...
						tetribodies[highestbody()+1] = newBody(world, tetribodies[i-ioffset]:getX(), tetribodies[i-ioffset]:getY(), tetribodies[i-ioffset]:getMass(), blockrot)
						...
						--mass confusion
						sMFSreplacement(tetribodies[highestbody()])
